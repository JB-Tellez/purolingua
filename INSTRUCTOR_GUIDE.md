# XP Workshop: Instructor Guide

A 5-day intensive workshop for working developers learning Extreme Programming practices through a real brownfield codebase — a vanilla JavaScript flashcard app that teams will port to React, strengthen with tests, and extend with new features.

See `STUDENT_GUIDE.md` for the student-facing version of this document.
...
- **Linting:** None (intentionally — students set this up)

---

## Workshop Structure

Iterations are sequential — each builds on the previous — but teams move through them at their own pace. There are no day assignments. The one fixed point is **Day 5**, which is reserved for wrapping up, showcase, and retrospective.
...
Architecture is open. Teams decide:
...
Questions teams will encounter:
...
Available for teams that move through the core iterations and want to go deeper. Teams can also pull these in earlier if they see fit.
...
Day 5 is the one fixed point on the calendar. No new feature work — teams use the morning to finish in-progress stories, clean up, and prepare to present.

**Team demos**
- Each team shows their working app
...
- Teams review each other's code
...
## Codebase Preparation Notes

Recommendations for preparing the repo before handing it to students:

1. **Consider removing `voice.js`** and speech recognition references from `app.js` (lines 3, 99, 389-481). It only works in webkit browsers (Chrome, Safari, Edge) — students on Firefox can't test it manually. The Web Speech Recognition API is also harder to mock in Vitest than most browser APIs. That said, writing tests for a flaky browser dependency is a legitimate real-world challenge. Keep it if you want that exercise; remove it if browser compatibility will cause friction in your classroom setup.

2. **Remove `data/decks.json`** — legacy file, unused. Decks come from locale files.

3. **Pre-install React or not** — adding `react`, `react-dom`, and `@testing-library/react` to `devDependencies` saves time. Leaving them out makes Iteration 2 more realistic but risks npm troubleshooting eating into workshop time.

4. **Verify tests pass** — run `npm test` and confirm all existing tests are green before students start.
